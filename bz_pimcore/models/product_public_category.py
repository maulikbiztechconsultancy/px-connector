...
class ProductCategoryRelation(models.Model):
    ...
    odoo_caegory_id = fields.Many2one(
        'product.public.category', string="Current Category",
    )
    pim_category_id = fields.Many2one('queue.job', string="Custom Value", domain="[('model_id', '=', 'product.public.category'), ('state', '=', 'process'), ('pim_config_id', '=', config_id)]")
...
class ProductCategory(models.Model):
    ...
    def create_queue_record_create(self):
        queue_categorys = self.env['queue.job'].search([('model_id', '=', 'product.public.category'), ('state', '=', 'process')], order='priority asc')
        category_parent = False
        for queue_category in queue_categorys:
            try:
                self.get_data_prepartion(queue_category)
        ...
        if category_parent:
            for queue_category in queue_categorys:
                self.set_parent_category(queue_category)

    def get_data_prepartion(self, queue_category=None):
    ...
    def set_parent_category(self, queue_category=None):
        pim_config_id = queue_category.pim_config_id
        filed_data = {'pim_config_id': pim_config_id.id}
        result = queue_category.result
        filed_data = {}
            ...
            for parents_data in self.search([('reference_id', 'in', parents)]):
                ...
                if not parent_category:
                    if not main_category.parent_id and not main_category.parent_id.id:
                        ...
                        main_category.copy(default={'parent_id': parents_data.id})
